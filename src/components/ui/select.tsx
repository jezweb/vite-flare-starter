...
/**
 * Radix's Select.Value rendered the selected item's label (its ItemText
 * JSX); Base UI renders the RAW VALUE unless the Root is given an `items`
 * map. To keep the radix behavior for every existing consumer, this
 * wrapper walks its JSX children and derives `items` from anything that
 * looks like an item (has both `value` and `children` props — this also
 * sees through thin wrappers like PromptInputSelectItem). Pass an
 * explicit `items` prop to skip the derivation, or pass children to
 * `SelectValue` to control the trigger label directly.
 */
function collectItemLabels(
  ...
  React.Children.forEach(node, (child) => {
    ...
    if (
      ...
      out[String(value)] = props.children
      return
    }
...
function Select<Value, Multiple extends boolean | undefined = false>({
  ...
  const derivedItems = items ?? collectItemLabels(children, {})
  return (
